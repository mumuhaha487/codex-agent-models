...
flowchart TD
    ...
    C --> D[一次性交付修改摘要与测试证据 UI含桌面移动端]
    D --> E{证据和范围满足合同?}
    E -- 否 --> F{任一失败计数达到 5?}
    F -- 否 --> G[把失败证据发给同一子智能体]
    G --> C
    F -- 是 --> H[丢弃隔离 worktree 并恢复任务基线]
    H --> I[主 Agent 重新拆分或改派任务]
    E -- 是 --> J[派发另一 CustomAgent 独立只读审查]
    J --> K{独立审查发现问题?}
    K -- 是 --> L[审查发现交回原实现者修订并复核]
    L --> C
    K -- 否 --> M[压缩为单个提交并整合]
    M --> N{整合产生新问题?}
    N -- 否 --> O[删除 worktree 分支 检查点与记录]
    N -- 是 --> P[交回相关子智能体或回退整合]
```
...
- 必须运行的测试、静态检查、构建或人工验证标准（涉及 UI 时包含桌面端与移动端真实视口验收证据）。
- 交付时需要返回的修改摘要、测试结果和剩余风险。
- 通信约束：自行选择具体实现、自行运行命令并等待结果；除实际阻塞、需要决定或影响其他任务外，中途不汇报。

主 Agent 不把具体实现拆成连续的小指令。任务范围或验收标准没有变化时，后续修订继续交给同一子智能体，以复用它已建立的代码上下文。独立审查必须由另一个独立的 `CustomAgent` 在独立上下文只读执行，审查发现交回原实现者修订后再复核。
...
- 主 Agent 只处理范围合规、任务间依赖和集成后新出现的问题。独立审查必须交给新的子智能体。
...
原始要求、测试和独立审查均满足交付合同时直接整合。只有整合改变了环境、出现冲突或暴露新失败时，才运行针对该新问题的检查；不要重跑子智能体已经成功执行的完整测试：
