...
6. 运行下面的单一入口，并把输出的 localhost 页面打开给用户。用户只填写模型、思考强度（`none`、`low`、`medium`、`high`）和是否支持识图；保存页面即确认应用，不再要求聊天中回复“已确认”。
...
10. 验收必须覆盖父 Provider 直连、显式 `spawn_agent(agent_type="CustomAgent", fork_context=false)` 可写子线程，以及未指定 `agent_type` 时实际使用页面选择的默认模型和思考强度（包括 `none`）。
11. 若结果要求重启或新任务，提示用户完全退出 Codex 后重新打开。
12. 可写任务使用 `scripts/task_worktree.py` 管理隔离、检查点、整合、回退和清理。主模型负责完整任务合同、协调、阻碍处理与集成，不代替实现者编码，也不代替独立审查者审查。实现子任务由 `CustomAgent` 在隔离 worktree 中完成，独立审查必须由另一个 `CustomAgent` 只读检查，审查发现交回原实现者修订并复核；界面任务必须包含桌面端与移动端真实视口验收证据。普通子智能体请求路由到 `CustomAgent`，仅授权调用，默认不覆盖模型或思考强度，禁止修改持久配置。禁止连续轮询和重复执行子智能体已成功运行的检查，等待按 `1 -> 2 -> 4 -> 8 -> 16` 分钟退避后维持 16 分钟。任一任务级失败计数达到 `5` 时，恢复基线并重新拆分或改派，不发起第 6 次相同尝试，也不由主模型接手实现。
