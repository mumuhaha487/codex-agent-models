...
4. 用 `scripts/codex_custom_agent.py status --json` 只读检查。缺少任一配置时，运行 `node <skill-dir>/scripts/credential-ui/src/profile.ts setup default`，把返回的 localhost 链接展示给用户并等待用户亲自保存 URL、Key、模型三项；不要代填、读取或回显值。URL 必须使用用户或网关给出的精确 Base URL，不得自行追加 `/v1` 或其他路径；用户给出的合法路径必须保留。
